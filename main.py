...
firebase_admin.initialize_app(cred, {
    ...
    'storageBucket': "sandyface-804db.appspot.com"
})
...
cap.set(3,640)  # these are for the image set into the bg  # the complete image is 1280 , 720
cap.set(4,480)  #comp image is 1280 720

imgBackground = cv2.imread('Resources/background.png')  # for the graphics import to the bg(with the file exten)

# Importing the mode images into the list  #As in loop formatq
...
# print(len(imgModeList)) -  the import have been done this stage of code.
...
while True:
    success, img = cap.read()  #if the image is done cap - read the img
    ...
    # to view them one by one in above.
    ...
    if faceCurFrame:
        ...
        if counter != 0:
            if counter == 1:
                ...
                # If now - timeLastAttendance is greater than 30 seconds  # we can change if we want byt the hrs
